...
export async function generateStaticParams() {
    return getArchiveHubsByLocale('ja').map(hub => ({ slug: hub.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    const { slug } = await params
    const hub = getArchiveHubByLocaleAndSlug('ja', slug)
    if (!hub) return {}
    const items = await getHubItems(hub)
    const itemCount = process.env.SKIP_BUILD_STATIC_GENERATION ? undefined : items.length
    return buildHubMetadata('ja', slug, { itemCount })
}

export default async function HubPageJa({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params
    const hub = getArchiveHubByLocaleAndSlug('ja', slug)
    if (!hub) notFound()

    const items = await getHubItems(hub)
    return <HubPageContent hub={hub} locale="ja" items={items} />
}
