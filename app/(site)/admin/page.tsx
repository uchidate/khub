...
import { getAdminDashboardStats } from '@/lib/repositories/AdminRepository'
...
export default async function AdminPage() {
  ...
  const { totals, new30d, newUsers7d, pipeline, pending, recent, sparks, now } = await getAdminDashboardStats()

  const totalUsers        = totals.users
  const totalArtists      = totals.artists
  const totalProductions  = totals.productions
  const totalNews         = totals.news
  const totalGroups       = totals.groups
  const newUsers          = new30d.users
  const newArtists        = new30d.artists
  const newProductions    = new30d.productions
  const newNews           = new30d.news
  const newGroups         = new30d.groups
  const newsImportedToday  = pipeline.imported
  const newsPublishedToday = pipeline.published
  const newsQueueToday     = pipeline.queue
  const newsHiddenToday    = pipeline.hidden
  const pendingReports     = pending.reports
  const flaggedComments    = pending.flaggedComments
  const recentUsers        = recent.users
  const recentNews         = recent.news
  const recentArtists      = recent.artists
  const recentProductions  = recent.productions

  const totalPendingTranslations = pending.artistBio + pending.productionTranslations
  const totalPendingEnrichment   = pending.artistsWithoutBio + pending.artistsWithoutImage + pending.groupsWithoutBio
  const operationalIncidents     = pending.systemErrors24h + pending.aiFailures24h
