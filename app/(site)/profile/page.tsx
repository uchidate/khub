...
function getRoleBadge(role: string) {
  switch (role?.toLowerCase()) {
    ...
    default:       return { label: 'Membro',        color: 'bg-[#ff2d78]/10 text-[#ff2d78] border-[#ff2d78]/20' }
  }
}
...
export default async function ProfilePage() {
  ...
  const miniStats = [
    { label: 'Artistas',   value: stats.favoriteArtists,    icon: Star,          color: 'text-[#ff2d78]',   bg: 'bg-[#ff2d78]/10' },
  ...
  return (
    <PageTransition>
      <div className="min-h-screen pb-20">
        ...
        {/* ── Hero Banner ─────────────────────────────────────────────────────── */}
        <div className="relative w-full h-52 md:h-72 overflow-hidden bg-gradient-to-br from-[#ff2d78]/10 via-[#f5f5f7] to-[#ff2d78]/5">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 -mt-24 relative z-10 space-y-6">
          ...
          <div className="glass-card p-6 md:p-8">
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-end">
              ...
              <div className="relative flex-shrink-0">
                {session.user.image ? (
                  <div className="relative w-28 h-28 md:w-32 md:h-32 rounded-2xl overflow-hidden border-4 border-[#ff2d78] shadow-2xl shadow-[#ff2d78]/20">
                    <Image
                  ...
                  <div className="w-28 h-28 md:w-32 md:h-32 rounded-2xl bg-[#ff2d78] flex items-center justify-center border-4 border-[#ff2d78] shadow-2xl shadow-[#ff2d78]/20">
              ...
              <div className="flex-1 text-center md:text-left">
                <p className="text-[#ff2d78] font-black tracking-widest uppercase text-[10px] mb-1">Perfil</p>
                <h1 className="text-3xl md:text-4xl font-display font-black text-foreground tracking-tight leading-none mb-3">
                  {session.user.name}
                </h1>
                <div className="flex flex-wrap items-center gap-2 justify-center md:justify-start mb-2">
                  ...
                  {memberSinceFormatted && (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs text-muted bg-surface border border-border">
                      <Calendar size={12} />
                ...
                {bio && (
                  <p className="text-muted text-sm mt-2 max-w-md">{bio}</p>
                )}
              </div>
              ...
              <div className="flex gap-3 flex-shrink-0">
                ...
                <Link
                  href="/settings"
                  className="flex items-center gap-2 px-4 py-2.5 bg-surface hover:bg-[#e8e8e8] text-foreground text-xs font-bold rounded-xl transition-colors border border-border uppercase tracking-widest"
                >
                  <Settings size={16} />
                ...
                {(session.user.role?.toLowerCase() === 'admin' || session.user.role?.toLowerCase() === 'editor') && (
                  <Link
                    href="/admin"
                    className="flex items-center gap-2 px-4 py-2.5 bg-[#080808] hover:bg-[#080808]/80 text-white text-xs font-bold rounded-xl transition-colors uppercase tracking-widest"
                  >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {miniStats.map(({ label, value, icon: Icon, color, bg }) => (
              <div key={label} className="glass-card p-4 flex items-center gap-3 rounded-xl border border-border">
                <div className={`p-2 rounded-lg ${bg} ${color}`}>
                  <Icon size={16} />
                </div>
                <div>
                  <p className="text-xl font-black text-foreground leading-none">{value.toLocaleString('pt-BR')}</p>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <section className="glass-card p-6">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 bg-[#ff2d78]/10 rounded-lg">
                        <Star size={16} className="text-[#ff2d78]" />
                      </div>
                      <div>
                        <h2 className="text-xl font-display font-black text-foreground uppercase italic tracking-tight">Artistas Favoritos</h2>
                        <p className="text-xs text-muted">Seus artistas recentes</p>
                      </div>
                    </div>
                    <Link href="/favorites" className="text-xs font-black text-muted hover:text-foreground uppercase tracking-widest transition-colors flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                    {recentFavoriteArtists.map((artist) => (
                      <Link key={artist.id} href={`/artists/${artist.id}`} className="group text-center">
                        <div className="relative w-full aspect-square rounded-xl overflow-hidden bg-surface mb-2 border border-border group-hover:border-[#ff2d78]/40 transition-all">
                          {artist.primaryImageUrl ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center bg-[#ff2d78]/5">
                              <User className="w-8 h-8 text-muted" />
                            </div>
                          )}
                        </div>
                        <p className="text-[10px] text-muted group-hover:text-foreground font-bold truncate transition-colors">
                ...
                <section className="glass-card p-6">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h2 className="text-xl font-display font-black text-foreground uppercase italic tracking-tight">Produções Salvas</h2>
                        <p className="text-xs text-muted">Na sua lista de favoritos</p>
                      </div>
                    </div>
                    <Link href="/favorites" className="text-xs font-black text-muted hover:text-foreground uppercase tracking-widest transition-colors flex items-center gap-2">
                    ...
                    {recentFavoriteProductions.map((prod) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-foreground group-hover:text-[#ff2d78] transition-colors truncate">
                            {prod.titlePt}
                          </p>
              ...
              {recentComments.length > 0 && (
                <section className="glass-card p-6">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <div>
                      <h2 className="text-xl font-display font-black text-foreground uppercase italic tracking-tight">Comentários Recentes</h2>
                      <p className="text-xs text-muted">Suas últimas interações</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {recentComments.map((comment) => (
                      <Link
                        ...
                        className="block p-4 rounded-xl bg-surface hover:bg-[#e8e8e8] border border-border hover:border-[#080808]/15 transition-all group"
                      >
                        <p className="text-sm text-muted line-clamp-2 mb-2 group-hover:text-foreground transition-colors">
                          &ldquo;{comment.content}&rdquo;
                        </p>
                        <div className="flex items-center gap-2">
                          <Newspaper className="w-3 h-3 text-muted" />
                          <p className="text-xs text-muted truncate group-hover:text-foreground transition-colors">
                            {comment.news.title}
                          </p>
                          <span className="ml-auto text-xs text-muted flex-shrink-0 flex items-center gap-1">
                            <Clock size={10} />
              ...
              {isEmpty && (
                <div className="glass-card p-12 text-center">
                  <Heart className="w-12 h-12 text-[#e8e8e8] mx-auto mb-4" />
                  <h3 className="text-xl font-display font-black text-foreground uppercase italic tracking-tight mb-2">Comece sua jornada!</h3>
            ...
            <div className="space-y-4">
              ...
              <section className="glass-card p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-xl font-display font-black text-foreground uppercase italic tracking-tight">Conquistas</h2>
                </div>
              ...
              {/* Links Rápidos */}
              <section className="glass-card p-6">
                <h2 className="text-xs font-black uppercase tracking-widest text-muted mb-3">Navegação</h2>
                <div className="space-y-1">
                  {[
                    ...
                    { href: '/dashboard',  icon: LayoutDashboard,label: 'Dashboard',      color: 'text-[#ff2d78]' },
                    { href: '/settings',   icon: Settings,        label: 'Configurações', color: 'text-muted' },
                    ...
                    >
                      ...
                      <span className="text-sm text-muted group-hover:text-foreground transition-colors font-bold">{label}</span>
                      <ChevronRight className="w-4 h-4 text-[#e8e8e8] group-hover:text-muted ml-auto transition-colors" />
                    </Link>
                  ))}
                </div>
              </section>
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              <div>
                <h2 className="text-2xl font-display font-black text-foreground uppercase italic tracking-tight">Minha Coleção</h2>
                <p className="text-muted text-sm mt-1">Todos os seus itens favoritos</p>
              </div>
              <Link href="/favorites" className="text-xs font-black text-muted hover:text-foreground uppercase tracking-widest transition-colors flex items-center gap-2">
