    ...
    <PageTransition>
      <main className="min-h-screen pb-16">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
          <header className="mb-5 overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
            <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_380px]">
              <section className="flex min-w-0 flex-col gap-4 p-5 sm:flex-row sm:items-center sm:p-6">
                <div className="relative flex-shrink-0">
                  {session.user.image ? (
                    <div className="relative h-24 w-24 overflow-hidden rounded-2xl border border-border bg-background sm:h-28 sm:w-28">
                      <Image src={session.user.image} alt={session.user.name ?? 'Perfil'} fill className="object-cover" sizes="112px" />
                    </div>
                  ) : (
                    <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-accent text-4xl font-black text-white sm:h-28 sm:w-28">
                      {session.user.name?.charAt(0).toUpperCase() ?? '?'}
                    </div>
                  )}
                  <span className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full border-4 border-surface bg-green-500" />
                </div>

                <div className="min-w-0 flex-1">
                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0">
                      <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-accent">Perfil</p>
                      <h1 className="max-w-3xl text-3xl font-black tracking-tight text-foreground sm:text-4xl">
                        {session.user.name}
                      </h1>
                    </div>
                    <div className="flex flex-wrap gap-2 xl:justify-end">
                      <EditProfileForm name={session.user.name ?? null} bio={bio} />
                      <Link href="/settings" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-border bg-background px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-foreground transition-colors hover:border-accent/40 hover:text-accent">
                        <Settings className="h-3.5 w-3.5" />
                        Ajustes
                      </Link>
                      {(session.user.role?.toLowerCase() === 'admin' || session.user.role?.toLowerCase() === 'editor') && (
                        <Link href="/admin" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-foreground px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-background transition-colors hover:bg-accent hover:text-white">
                  ...
                  <p className="mt-3 max-w-3xl text-sm leading-5 text-muted">
                    {bio ?? 'Adicione uma bio curta para apresentar sua curadoria, seus interesses e seu jeito de acompanhar o universo Hallyu.'}
                  </p>
                </div>
              </section>

              <aside className="border-t border-border bg-background p-4 sm:p-5 lg:border-l lg:border-t-0">
                <div className="grid grid-cols-2 gap-2">
                  {collectionStats.map(({ label, value, icon: Icon }) => (
                    <div key={label} className="rounded-2xl border border-border bg-surface p-3">
                      ...
                      <p className="text-xl font-black leading-none text-foreground">{value.toLocaleString('pt-BR')}</p>
                      <p className="mt-1 truncate text-[9px] font-black uppercase tracking-[0.12em] text-muted">{label}</p>
                    </div>
                  ))}
                </div>
              </aside>
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
              <div className="space-y-5">
                ...
                {recentFavoriteProductions.length > 0 && (
                  <section>
                    <SectionHeader title="Produções favoritas" subtitle="Dramas e filmes salvos na sua coleção." href="/favorites" />
              ...
              <aside className="space-y-5">
                <section className="rounded-2xl border border-border bg-surface p-4 shadow-sm">
                  <SectionHeader title="Marcos" subtitle="Sinais de participação no site." />
                  {achievements.length > 0 ? (
                    ...
                    <p className="rounded-2xl border border-dashed border-border bg-background p-4 text-sm leading-6 text-muted">
                      Interações futuras aparecem aqui como marcos.
                    </p>
                  )}
                </section>
