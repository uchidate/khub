...
export async function generateStaticParams() {
    return getArchiveHubsByLocale('fil').map(hub => ({ slug: hub.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    const { slug } = await params
    const hub = getArchiveHubByLocaleAndSlug('fil', slug)
    if (!hub) return {}
    const items = await getHubItems(hub)
    const itemCount = process.env.SKIP_BUILD_STATIC_GENERATION ? undefined : items.length
    return buildHubMetadata('fil', slug, { itemCount })
}

export default async function HubPageFil({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params
    const hub = getArchiveHubByLocaleAndSlug('fil', slug)
    if (!hub) notFound()

    const items = await getHubItems(hub)
    return <HubPageContent hub={hub} locale="fil" items={items} />
}
