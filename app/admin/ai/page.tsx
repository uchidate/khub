            ...
            <div className="space-y-6">
                {loadError && (
                    <div className="bg-red-900/20 border border-red-700/40 rounded-xl p-4">
                        ...
                        <p className="text-xs text-muted mt-2">Verifique se as tabelas <code>ai_usage_log</code> e <code>ai_config</code> existem no banco. Execute <code>prisma migrate deploy</code> se necessário.</p>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-1">Chamadas</p>
                        <p className="text-2xl font-bold text-foreground tabular-nums">{summary.totalCalls.toLocaleString()}</p>
                        <p className="text-xs text-muted mt-0.5">{summary.errorCalls} erro{summary.errorCalls !== 1 ? 's' : ''}</p>
                    </div>
                    <div className="bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-1">Taxa de sucesso</p>
                        ...
                        <p className="text-xs text-muted mt-0.5">{summary.successCalls} sucessos</p>
                    </div>
                    <div className="bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-1">Custo total</p>
                        <p className="text-2xl font-bold text-foreground tabular-nums">{fmtCost(summary.totalCostUsd)}</p>
                        <p className="text-xs text-muted mt-0.5">mês: {fmtCost(monthlySpend.total)}</p>
                    </div>
                    <div className="bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-1">Latência média</p>
                        <p className="text-2xl font-bold text-foreground tabular-nums">{fmtMs(summary.avgLatencyMs)}</p>
                        <p className="text-xs text-muted mt-0.5">por chamada</p>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2 bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-4">Custo — 14 dias</p>
                        {costByDay.length === 0 ? (
                            <p className="text-xs text-muted py-8 text-center">Nenhum dado ainda</p>
                        ) : (
                                ...
                                {costByDay.map(d => {
                                    ...
                                    return (
                                        <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group">
                                            ...
                                            <span className="text-[8px] text-muted group-hover:text-muted transition-colors">{label}</span>
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        {/* Calls sub-chart */}
                        {costByDay.length > 0 && (
                            <div className="flex items-end gap-1 h-10 mt-2 border-t border-border pt-2">
                                {costByDay.map(d => (
                                    <div key={d.date} className="flex-1 flex flex-col items-center">
                                        <div
                                            className="w-full bg-surface hover:bg-surface-hover rounded-t transition-colors cursor-default"
                        ...
                        <p className="text-[9px] text-muted mt-1">barras cinzas = número de chamadas</p>
                    </div>

                    {/* Feature breakdown */}
                    <div className="bg-surface border border-border rounded-xl p-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-4">Por feature</p>
                                    ...
                                    return (
                                        <div key={feat}>
                                            <div className="flex justify-between text-[10px] mb-1">
                                                <span className={`${over ? 'text-red-400' : 'text-muted'} truncate mr-2`}>{label}</span>
                                                <span className="text-muted shrink-0">{count} ({pct}%)</span>
                                            </div>
                ...
                <div className="bg-surface border border-border rounded-xl p-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-3">Custo por provider (30d)</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        {Object.entries(summary.callsByProvider).map(([prov, calls]) => {
                            const cost  = summary.costByProvider[prov] ?? 0
                            const color = providerColors[prov] ?? 'bg-border'
                            return (
                                <div key={prov} className="bg-surface rounded-lg p-3 border border-border">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className={`w-2 h-2 rounded-full ${color}`} />
                                        <span className="text-xs font-semibold text-foreground capitalize">{prov}</span>
                                    </div>
                                    <p className="text-sm font-bold text-foreground tabular-nums">{fmtCost(cost)}</p>
                                    <p className="text-[10px] text-muted">{calls} chamadas</p>
                                </div>
                            )
                        })}
                        {Object.keys(summary.callsByProvider).length === 0 && (
                            <p className="col-span-full text-xs text-muted py-2">Nenhuma chamada registrada ainda</p>
                        )}
                    </div>
                </div>
