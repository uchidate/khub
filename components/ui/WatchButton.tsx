    ...
    return (
        <div className={`relative ${className}`} ref={menuRef}>
            <button
                onClick={handleOpen}
                disabled={!isLoaded}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold text-white transition-colors ${buttonBg} ${!isLoaded ? 'opacity-50 cursor-wait' : ''}`}
            ...
            {open && (
                <div className="absolute z-50 mt-1 w-72 rounded-xl border border-zinc-700 bg-zinc-900 shadow-xl">
                    {/* Status options */}
                    <div className="p-2">
