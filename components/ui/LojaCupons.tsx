        ...
        <section className={compact ? '' : 'mt-10'}>
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-caption font-semibold bg-green-500/10 text-green-500 border border-green-500/20 px-2 py-0.5 rounded-full">
                ...
                {coupons.map(c => (
                    ...
                    >
                        ...
                        <div className="flex-shrink-0 bg-orange-500/10 border border-orange-500/20 border-dashed rounded-lg px-3 py-2 text-center min-w-[80px]">
                            ...
                            <p className="text-caption font-bold text-orange-400">{c.discount}</p>
                        </div>
                        {/* Info */}
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-foreground line-clamp-2 leading-snug">{c.description}</p>
                            {c.minPurchase && <p className="text-caption text-muted mt-0.5">{c.minPurchase}</p>}
                            <p className="text-caption text-muted mt-0.5">{STORE_LABELS[c.store] ?? c.store}</p>
                        </div>
