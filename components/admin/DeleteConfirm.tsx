...
export function DeleteConfirm({ open, count, entityName, onClose, onConfirm }: DeleteConfirmProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-background border border-border rounded-2xl w-full max-w-sm p-6 text-center shadow-2xl">
        ...
        <h3 className="text-base font-bold text-foreground mb-1.5">Confirmar exclusão</h3>
        {error && (
        ...
        <div className="flex gap-2.5">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-2.5 bg-surface text-foreground rounded-xl hover:bg-surface-hover disabled:opacity-50 transition-colors text-sm font-medium"
          >
            Cancelar
          </button>
          <button
