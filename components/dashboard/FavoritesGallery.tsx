...
const TYPE_CONFIG = {
  artist:     { label: 'Artista',  icon: User,      color: 'text-[#ff2d78]', bg: 'bg-[#ff2d78]/10' },
...
function FavoriteCard({ item }: { item: FavoriteItem }) {
  ...
  return (
    <Link href={href} className="group block">
      {/* Poster */}
      <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-[#f5f5f7] border border-[#e8e8e8] group-hover:border-[#080808]/20 transition-all duration-300 mb-2 shadow-lg">
        {imageUrl ? (
          <Image
            src={imageUrl}
            alt={name}
      ...
      {/* Name */}
      <p className="text-xs font-bold text-[#080808] truncate group-hover:text-[#ff2d78] transition-colors leading-tight">
        {name}
      </p>
      {subtitle && (
        <p className="text-[10px] text-[#6b6b6b] truncate mt-0.5">{subtitle}</p>
      )}
    </Link>
  )
}
    ...
    return (
      <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-3">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="aspect-[2/3] animate-pulse bg-[#f5f5f7] rounded-xl" />
        ))}
      </div>
    )
  }
  ...
  if (items.length === 0) {
    return (
      <div className="space-y-6">
        <div className="glass-card p-8 text-center border-dashed border-2 border-[#e8e8e8] flex flex-col items-center justify-center">
          <div className="w-16 h-16 bg-[#f5f5f7] rounded-full flex items-center justify-center mb-4">
            <Star size={32} className="text-[#e8e8e8]" />
          </div>
          <h3 className="text-lg font-bold text-[#080808] mb-2">Sua galáxia está vazia</h3>
          <p className="text-[#6b6b6b] mb-6 max-w-sm mx-auto text-sm">
        ...
        {trendingArtists.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black uppercase tracking-widest text-[#6b6b6b]">Em alta agora</h3>
            </div>
              ...
              {trendingArtists.map((artist) => (
                <Link key={artist.id} href={`/artists/${artist.slug ?? artist.id}`} className="group block">
                  <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-[#f5f5f7] border border-[#e8e8e8] group-hover:border-[#080808]/20 transition-all mb-2">
                    ...
                    <div className="absolute bottom-1.5 left-1.5 flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black bg-[#ff2d78]/10 text-[#ff2d78] backdrop-blur-sm">
                      <User size={7} />Artista
                    </div>
                  </div>
                  <p className="text-xs font-bold text-[#080808] truncate group-hover:text-[#ff2d78] transition-colors">{artist.nameRomanized}</p>
                </Link>
              ))}
            </div>
          </div>
